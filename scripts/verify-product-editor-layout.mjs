...
try {
  ...
  assert.equal(await page.$eval('[data-video-title]', el => el.getAttribute('aria-invalid')), 'true');
  assert.match(await page.$eval('[data-video-title-help]', el => el.textContent), /缺少标题/);
  ...
  pass('the actual screenshot condition is explicit: valid link plus empty title is a saved draft, not published content');
  ...
  assert.equal(await page.$eval('[data-testid="confirm-product-publication"]', el => el.disabled), true);
  await click('[data-testid="product-publication-issues"] button');
  await page.waitForFunction(() => document.activeElement?.matches('[data-video-title]'));
  assert.equal((await data()).releases.length, 0);
  pass('persistent actions and footer lead to product-only confirmation; missing title has a direct field correction link');
  ...
  await click('[data-testid="confirm-product-publication"]'); await click('[data-testid="publish-product"]'); await page.waitForSelector('[data-testid="product-publication-success"]');
  ...
  const html = await (await request(base + '/zh/products/open-play')).text(); assert.match(html, /新的完整商品介绍与视频说明/); assert.match(html, /data-video-id="video-screenshot"/);

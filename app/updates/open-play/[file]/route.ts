...
export async function GET(request: NextRequest, context: Context) {
  return serveDownload(request, await resolveOpenPlayDownload((await context.params).file, true), false);
}
export async function HEAD(request: NextRequest, context: Context) {
  return serveDownload(request, await resolveOpenPlayDownload((await context.params).file, true), true);
}

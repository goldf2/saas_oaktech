...
操作：编辑商品 → 商品资料 → 添加视频 → 填写标题和YouTube/B站完整视频链接 → 可上传封面、继续添加其他来源 → 保存草稿 → 预览发布 → 明确确认。
...
- `components/product-videos.tsx`：公开/预览共用的按需播放器。没有点击前不创建iframe；切换来源或离开预览销毁旧播放器。支持200px最小播放器尺寸、手机布局和键盘操作。
...
- 视频资料被包含在草稿/发布指纹，过期编辑和过期确认被拒。未完成视频可以存草稿，但最终发布必须补齐标题与有效来源。

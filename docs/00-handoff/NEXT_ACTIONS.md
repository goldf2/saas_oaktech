...
**唯一下一开发任务：ADM-02。** 先读[HANDOFF.md](HANDOFF.md)，认领后实现正式事务schema、显式新安装准备与安装标识核对，不再继续用反复手填subject的办法代替已确认的新功能。

后续依赖顺序：ADM-02事务schema → ADM-03身份登记 / ADM-04初始化凭据 → ADM-05原子绑定 → ADM-06页面 / ADM-07角色服务 → ADM-08用户权限页 / ADM-09管理写接入 → ADM-10迁移恢复 → ADM-11容器演练 → ADM-12隔离验收 → ADM-13预生产真账号 → ADM-14生产真账号。实际并行条件以[TASKS.json](TASKS.json)为准。

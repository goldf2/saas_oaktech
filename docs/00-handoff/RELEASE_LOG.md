...
## 0.1.26 · 项目计划和开发接续（2026-09-16T15:59:17+08:00）

- 变更仅限项目文档、进度生成/校验及其测试，package与lockfile同步0.1.26；未实现业务超管功能。
- 新增21项任务与40条验收场景，采用CT-ADMIN-001固定仓库快照，原文档与过期状态归档。
- 基线0.1.25 / 54a42b3，版本推送不代表账号授权或软件包发布。最终验证见 `evidence/2026-09-16-plan-validation.json`。
- 代码推送、CI测试、部署job与实际公网版本需分别记录；部署后收据保存 `.local-verification/0.1.26/plan-delivery.json`。
- 回滚：本版本不改变业务schema、账号或软件目录；源码回滚不得清理持久数据。
...
## 0.1.31 · 商品介绍视频源（2026-09-18T09:04:03+08:00）

- 基于实际remote main 0.1.30/8616682，保留新版本发布UI；商品资料合并为三分组，加入多视频/多来源/封面、YouTube和B站内嵌、未知平台外链及草稿发布边界。
- 无新第三方依赖、无生产数据迁移；不自动为任何商品创建视频、不修改软件版本或用户权限。
- 验证包含124项回归、类型、默认Turbopack构建、新视频浏览器和旧工作台/发布UI。外部真实播放与确定性UI测试分开记录；精确证据见本轮验收文件。
- 实际push/CI/public health收据在.local-verification/0.1.31；不把版本号提前写入视为已经部署。
- 回滚代码至8616682需保留持久数据；旧版不会展示视频，旧保存界面可能丢弃未知字段，回滚期间暂停编辑含视频商品。

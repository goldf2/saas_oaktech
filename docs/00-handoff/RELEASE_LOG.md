...
移除顶部和手机主菜单的浏览器扩展入口；保留产品、支持与目录平台筛选。部署收据独立保存，未修改生产商品。
...
候选0.1.34：合并软件目录与商品管理，单一角色安全列表、公共详情/下载、后台操作及旧目录入口兼容；156单元/45浏览器通过。提交/生产观察按后续收据记录，不预报上线。


## 2026-09-19T06:04:00+08:00 · LANG-01

0.1.35候选：恢复自动语言协商、手动偏好、工作台导航和手机切换；等待远端和公网读回。
...
2026-09-20T08:52:36+08:00：082865b 推送成功；CI 验证通过、部署缺凭据失败。公网0.1.44，MCP路由404。


## 2026-09-20T09:34:58+08:00 0.1.46 MCP后台接入
216 tests、类型、构建及隔离CUA UI通过；后台创建/撤销令牌、权限/商品选择、模板下载和文档已实现。生产部署待验证，无生产令牌或业务数据变更。

2026-09-20T09:38:18+08:00：95c9aa9推送成功；CI验证通过，部署失败，公网仍0.1.44。

...
export function PublicationReview({ product, productIsPublished, onFixProduct, releases, selected, setSelected, dirty, releaseDirty, existing, disabled, confirmed, setConfirmed, onPublish, onBack, mode, onModeChange }: {
  ...
  return <section data-testid="publication-review" data-publication-scope={mode} className="min-w-0 space-y-5 rounded-xl border bg-background p-5 sm:p-6">
    <div><h2 className="text-xl font-semibold">独立预览与发布</h2><p className="mt-2 text-sm text-muted-foreground">商品资料和软件版本分别确认、分别发布。一次操作只改变当前选中的发布对象。</p></div>
    <div className="grid gap-3 sm:grid-cols-2" role="group" aria-label="选择发布对象">
      <button type="button" data-publication-mode="product" aria-pressed={!software} disabled={disabled} onClick={() => onModeChange("product")}
        className="rounded-xl border p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring aria-pressed:border-primary aria-pressed:bg-primary/5 disabled:opacity-50">
        <span className="flex items-center gap-2 font-semibold"><Globe className="h-5 w-5" aria-hidden="true" />商品资料发布</span><span className="mt-2 block text-sm text-muted-foreground">名称、介绍、图片与视频；不发布或切换软件版本。</span>
      </button>
      <button type="button" data-publication-mode="software" aria-pressed={software} disabled={disabled} onClick={() => onModeChange("software")}
        className="rounded-xl border p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring aria-pressed:border-primary aria-pressed:bg-primary/5 disabled:opacity-50">
        <span className="flex items-center gap-2 font-semibold"><PackageCheck className="h-5 w-5" aria-hidden="true" />软件版本发布</span><span className="mt-2 block text-sm text-muted-foreground">版本、安装包和更新清单；不公开或保存商品资料草稿。</span>
      </button>
    </div>
    {!software ? <>
      <p data-testid="metadata-only-notice" className="rounded-lg bg-primary/5 p-4 text-sm">本次仅发布商品资料。已有软件版本和下载保持不变，待上传、待校验或未保存的软件版本不会阻止资料发布。</p>
      ...
      <p data-testid="software-only-notice" className="rounded-lg bg-primary/5 p-4 text-sm">本次只发布选中的软件版本。商品介绍、图片、视频及其草稿保持原样；无需先保存、完善或发布正在编辑的商品资料。</p>
      {!productIsPublished && <p data-testid="unlisted-product-release-warning" className="rounded-lg border border-amber-300 p-4 text-sm">商品尚未上架。软件发布后安装包可通过其正式下载地址获取，但不会自动公开商品介绍页。需要展示商品时，请另行发布商品资料。</p>}
        ...
        return <div key={release.id} className={`min-w-0 rounded-xl border p-4 ${checked ? "border-primary bg-primary/5" : ""}`}>
          <label className="flex items-start gap-3 text-sm"><input className="mt-1 h-4 w-4 shrink-0 accent-primary" type="checkbox" data-release-select={release.id} checked={checked} disabled={disabled || releaseDirty || !readiness.ready} onChange={e => setSelected(selectReleaseForPublication(selected, release.id, e.target.checked, releases))} />
            <span className="min-w-0 flex-1"><strong className="text-base">{release.version}</strong><span className="ml-2 rounded-full bg-muted px-2 py-0.5 text-xs">{release.channel}</span>
              ...
              <span className="mt-2 block whitespace-pre-wrap break-words text-muted-foreground">{release.notes_zh || release.notes_en || "尚未填写更新说明"}</span>
            </span>
          </label>
    ...
    <div className="border-t pt-5">
      ...
      <div className="mt-4"><Button data-testid={software ? "publish-software" : "publish-product"} onClick={onPublish} disabled={!canConfirm || !confirmed}>{disabled ? "正在处理，请稍候…" : software ? `校验并发布 ${chosen.length} 个软件版本` : "仅发布商品资料"}</Button></div>
    </div>
  </section>;
}

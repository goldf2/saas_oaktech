...
import { RemoteReleaseImport, type GithubRelease } from "./remote-release-import";
...
import { formatBytes, releaseReadiness } from "@/lib/store/release-workflow";
...
function ReleaseEditor({ product, release }: { product: AdminStoreProductRow; release?: AdminProductReleaseRow }) {
  ...
  function fillGithub(source: GithubRelease) {
    setGithubSource(source.source);
    setValue({ version: source.version, channel: source.channel, title_zh: source.title, notes_zh: source.notes, title_en: source.title, notes_en: source.notes });
    setSameEnglish(true);
  }
  ...
  const [state, action, pending] = useActionState<AdminActionResult, FormData>(async (_previous, form) => {
    try {
      ...
      if (!result.error && result.releaseId && result.productSlug === product.slug) {
        ...
        else router.replace(`/admin/products/${encodeURIComponent(product.slug)}?tab=versions&release=${encodeURIComponent(result.releaseId)}&saved=1${githubSource ? `&source=${encodeURIComponent(githubSource)}` : ""}`, { scroll: false });
      }
      return result;
  ...
  return <div className="mt-2 space-y-3">
    {!release && <RemoteReleaseImport productSlug={product.slug} version={value.version} disabled={pending || operationBusy} onRelease={fillGithub} />}
    <div className="grid min-w-0 items-start gap-3">
      <details open={!release || dirty} className="min-w-0 rounded-lg border bg-background p-3 sm:p-4">
        <summary className="cursor-pointer text-sm font-medium">{release ? `${release.version} · ${zh ? release.title_zh : release.title_en} · ${zh ? "编辑版本资料" : "Edit release details"}` : (zh ? "填写版本资料" : "Release details")}</summary>
      ...
      {!published && release && <RemoteReleaseImport releaseId={release.id} productSlug={product.slug} version={release.version} channel={release.channel} artifacts={release.release_artifacts} disabled={dirty || pending || operationBusy} />}

    </div>
    {release && <>
      <section data-testid="version-files" className="min-w-0 rounded-lg border bg-background p-3 sm:p-4">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">{zh ? "版本文件" : "Release files"} <span className="ml-1 text-sm font-normal text-muted-foreground">{release.release_artifacts.length} {zh ? "个" : "files"} · {formatBytes(readiness!.totalBytes)}</span></h3><p className="mt-1 text-xs text-muted-foreground">{published ? (zh ? "这个版本已发布，下方文件可以公开下载。" : "This version is published; the files below are publicly downloadable.") : (zh ? "导入完成后，请检查文件并确认发布。" : "Administrator-only. A received file is not yet signature-approved or publicly published.")}</p></div><span className={`rounded-full px-3 py-1 text-xs ${published ? "bg-primary/10 text-primary" : "bg-amber-50 text-amber-800 dark:bg-amber-950/30 dark:text-amber-200"}`}>{published ? (zh ? "已发布" : "Published") : readiness?.ready ? (zh ? "文件资料齐备 · 待最终校验" : "Files ready · final verification pending") : (zh ? "尚未准备齐全" : "Not ready")}</span></div>
...
export function ProductReleases({ product, releases, selectedRelease, notice }: { product: AdminStoreProductRow; releases: AdminProductReleaseRow[]; selectedRelease?: string; notice?: string }) {
  ...
  return <section data-testid="product-releases" className="min-w-0 space-y-3">
    ...
    <details key={`new-${releases.length}`} data-testid="new-product-release" open={!releases.length} className="rounded-lg border bg-muted/10 p-3"><summary className="cursor-pointer list-none font-semibold"><span className="inline-flex items-center gap-2"><Plus className="h-5 w-5 text-primary" aria-hidden="true" />{zh ? "导入 / 新建软件版本" : "Import / create release"}</span><span className="ml-3 text-xs font-normal text-muted-foreground">{zh ? "保存为草稿，上传后再确认发布" : "Save as draft, upload files, then confirm publication"}</span></summary><ReleaseEditor product={product} /></details>

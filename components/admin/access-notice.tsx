...
export async function AdminAccessNotice() {
  ...
  return (
    <section className="container max-w-3xl px-4 py-12" data-testid="admin-access-notice">
      <p className="text-sm text-primary">OakTech · 商品后台</p>
      <h1 className="mt-3 text-3xl font-bold">{user ? "当前账号没有商品管理权限" : "请先登录商品后台"}</h1>
      <p className="mt-4 text-muted-foreground">商品添加、编辑和软件发布页面已提供，仅向已授权管理员开放；普通用户的个人中心不具备商品发布权限。</p>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <Button asChild><Link href={user ? "/dashboard" : "/sign-in"}>{user ? "返回个人中心" : "登录 / 注册"}</Link></Button>
        <Button asChild variant="outline"><Link href="/zh">返回软件商城</Link></Button>
      </div>
      {!user && <p className="mt-4 text-sm text-muted-foreground">登录后可从个人中心的「商品后台」入口继续。</p>}
    </section>
  );
}

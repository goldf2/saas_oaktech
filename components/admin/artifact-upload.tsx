...
export function ArtifactUpload({ releaseId, productSlug = "", version = "", artifacts = [], disabled = false, disabledReason = "正在处理，请稍候再上传。" }: { releaseId: string; productSlug?: string; version?: string; artifacts?: ArtifactRow[]; disabled?: boolean; disabledReason?: string }) {
  ...
  return <section data-testid="artifact-upload" className="min-w-0 rounded-xl border bg-background p-5" aria-label="上传安装包与更新清单">
    ...
    <form onSubmit={upload} aria-busy={busy} className="mt-4 min-w-0 space-y-4">
      <div className={`relative rounded-xl border-2 border-dashed p-6 text-center transition-colors ${dragging ? "border-primary bg-primary/10" : "border-primary/35 bg-primary/5"} ${disabled ? "opacity-60" : ""}`} onDragOver={e => { e.preventDefault(); if (!disabled && !busy) setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={e => { e.preventDefault(); setDragging(false); select(e.dataTransfer.files); }}>
        <UploadCloud className="mx-auto h-9 w-9 text-primary" aria-hidden="true" /><p className="mt-2 font-medium">把软件文件拖到这里</p><p className="mt-1 text-xs text-muted-foreground">支持多选 · 单文件最多 4 GiB · 每次最多 12 个</p>
        ...
        <Button type="button" className="mt-4" disabled={busy || disabled} onClick={() => fileInput.current?.click()}><UploadCloud className="mr-2 h-4 w-4" aria-hidden="true" />选择软件文件</Button>
        {disabled && !busy && <p className="mt-3 text-sm font-medium">{disabledReason}</p>}
      </div>
      {items.map(item => <div key={item.id} data-queue-file={item.file.name} className="min-w-0 rounded-lg border p-3">
        ...
        <fieldset disabled={busy || disabled || item.state === "uploaded" || item.state === "uncertain"} className="mt-3 grid min-w-0 gap-3 sm:grid-cols-3">
          <label className="min-w-0 text-xs">平台<Input name="platform" list={`${inputId}-platforms`} className="mt-1" value={item.platform} placeholder="选择平台" required onChange={e => patch(item.id, { platform: e.target.value })} /></label>
          <label className="min-w-0 text-xs">架构<Input name="architecture" list={`${inputId}-architectures`} className="mt-1" value={item.architecture} placeholder="例如 arm64" required onChange={e => patch(item.id, { architecture: e.target.value })} /></label>
          <label className="min-w-0 text-xs">包类型<Input name="package_kind" list={`${inputId}-kinds`} className="mt-1" value={item.packageKind} placeholder="例如 zip" required onChange={e => patch(item.id, { packageKind: e.target.value })} /></label>
        </fieldset>
      ...
      <div className="flex flex-wrap gap-2"><Button type="submit" disabled={busy || disabled || !pending.length}>{busy ? "上传与校验中…" : `开始上传${pending.length ? `（${pending.length} 个文件）` : ""}`}</Button>{busy && <Button type="button" variant="outline" onClick={() => { stopAfterCurrent.current = true; setMessage("当前文件完成后停止，不中断服务器正在提交的文件。"); }}>停止后续上传</Button>}{!busy && <Button type="button" variant="outline" onClick={() => router.refresh()}><RefreshCw className="mr-1.5 h-4 w-4" aria-hidden="true" />刷新文件列表</Button>}</div>
      {message && <p role={failed ? "alert" : "status"} className={`whitespace-pre-wrap break-words rounded-lg p-3 text-sm ${failed ? "bg-destructive/5 text-destructive" : "bg-muted/40 text-muted-foreground"}`}>{message}</p>}

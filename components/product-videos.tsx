...
import { videoSourceInfo, type ProductVideo } from "@/lib/store/product-videos";
...
function VideoPresentation({ video, locale }: { video: ProductVideo; locale: Locale }) {
  ...
  const title = video.title || (zh ? "视频介绍" : "Video introduction");
  const sourceName = source?.label || info?.label || "";
  const playing = Boolean(embed && loaded === `${source.id}:${embed}`);
  return <article className="min-w-0 overflow-hidden rounded-2xl border border-[hsl(var(--store-line))] bg-[hsl(var(--store-surface))]" data-video-id={video.id}>
    <div className="p-4 sm:p-5">
      ...
      {sources.length > 1 && <div role="group" aria-label={zh ? `${title}播放来源` : `${title} video sources`} className="mt-3 flex flex-wrap gap-2">
        {sources.map(item => <button key={item.id} type="button" data-video-source={item.id} aria-pressed={source?.id === item.id}
          onClick={() => { if (source?.id !== item.id) { setChosenId(item.id); setLoaded(null); } }}
    ...
    <div className="relative aspect-video min-h-[200px] w-full bg-muted/40">
      {playing ? <iframe key={`${source.id}:${embed}`} src={embed!} title={`${title} — ${sourceName}`} data-video-player={info!.provider}
        className="absolute inset-0 h-full w-full border-0" allow="encrypted-media; fullscreen; picture-in-picture"
        ...
        : <div className="absolute inset-0 flex flex-col items-center justify-center overflow-hidden p-5">
          ...
          <div className="relative flex max-w-full flex-col items-center gap-3 rounded-2xl bg-background/95 px-5 py-4 text-center shadow-sm">
            {embed ? <>
              <button type="button" data-video-load={video.id} onClick={() => setLoaded(`${source.id}:${embed}`)}
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-primary px-6 py-3 font-semibold text-primary-foreground hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                <Play className="h-5 w-5" aria-hidden="true" />{zh ? "加载播放器" : "Load video player"}
              </button>
              <p className="text-xs text-muted-foreground">{zh ? `通过 ${sourceName} 在本页观看；点击后连接该平台。` : `Watch here with ${sourceName}. The platform loads after your click.`}</p>
            </> : info ? <>
    ...
    {info && <div className="flex flex-wrap items-center justify-between gap-3 p-4 text-xs text-muted-foreground">
      ...
      {playing && <button type="button" onClick={() => setLoaded(null)} className="underline underline-offset-4">{zh ? "关闭播放器" : "Close player"}</button>}
    </div>}
  </article>;
}
